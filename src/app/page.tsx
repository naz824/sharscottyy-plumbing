...
import {
  ...
  BadgeDollarSign, Paintbrush, CheckCircle2, ArrowRight, ChevronUp,
} from "lucide-react";
import LeadCaptureForm from "@/components/LeadCaptureForm";
import FaqAccordion from "@/components/FaqAccordion";
import BackToTop from "@/components/BackToTop";
import { BUSINESS, SERVICES, TESTIMONIALS, FAQS, COMPETITOR_COMPARISON } from "@/lib/constants";
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* HERO — Fix #2: Real Yelp photo as hero background */}
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
              {COMPETITOR_COMPARISON.map((comp, i) => {
                const isUs = comp.company === "Sharscottyy Plumbing";
                return (
                  <div
                    key={comp.company}
                    className={`flex items-center justify-between px-6 py-4 ${
                      i < COMPETITOR_COMPARISON.length - 1 ? "border-b border-slate-100" : ""
                    } ${isUs ? "bg-green-50 border-2 border-green-200 rounded-2xl mx-2 my-2" : ""}`}
                  >
                    <span className={`font-bold ${isUs ? "text-green-700 text-lg" : "text-slate-600"}`}>
                      {isUs && "★ "}{comp.company}
                    </span>
                    <span className={`font-black text-lg ${isUs ? "text-green-600" : "text-red-500"}`}>
                      {comp.dispatchFee}
                    </span>
                  </div>
                );
      ...
      {/* WHY US — Fix #4: added id="why-us" */}
      <section id="why-us" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <p className="text-orange-600 font-black tracking-widest uppercase mb-3 text-sm">Why Locals Choose Us</p>
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 tracking-tight leading-[1.05]">
              Trust, Value &amp; Local Expertise
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <div key={feature.title} className="bg-slate-50 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_50px_rgba(8,112,184,0.12)] hover:-translate-y-1 transition-all duration-500 p-8 text-center border border-slate-100">
      ...
      {/* TESTIMONIALS — Fix #4: added id="reviews" */}
      ...
      {/* FAQ — Fix #10: reduced to 4 questions on homepage */}
      <section id="faqs" className="py-24 bg-white pb-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
